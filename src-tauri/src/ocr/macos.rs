...
fn make_cgimage(img: &RgbaImage) -> AppResult<objc2_core_foundation::CFRetained<CGImage>> {
    let (w, h) = img.dimensions();
    let data = CFData::from_bytes(img.as_raw());
    let provider = unsafe { CGDataProvider::with_cf_data(Some(&data)) }
        .ok_or_else(|| AppError::Ocr("CGDataProvider failed".into()))?;
    let space = unsafe { CGColorSpace::new_device_rgb() }
        .ok_or_else(|| AppError::Ocr("color space failed".into()))?;
    let bitmap_info = CGBitmapInfo(CGImageAlphaInfo::Last.0 | CGBitmapInfo::ByteOrderDefault.0);
    unsafe {
        CGImage::new(
            w as usize,
            h as usize,
